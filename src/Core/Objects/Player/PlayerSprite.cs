using Engine.ObjectGame.Animations;
...
public class PlayerSprite : BaseObjectGame
{
    ...
    {
        _texture = texture;
        AddBoundingBox(new Engine.ObjectGame.BoundingBox(new Vector2(BB1PosX, BB1PosY), BB1Width, BB1Height));
        AddBoundingBox(new Engine.ObjectGame.BoundingBox(new Vector2(BB2PosX, BB2PosY), BB2Width, BB2Height));
    ...
    public void MoveUp()
    {
        _movingRight = false;
        _movingLeft = false;
        _currentAnimation = _turnRightAnimation ?? _turnLeftAnimation;
        _rightToCenterAnimation.Reset();
        _turnLeftAnimation.Reset();
